...
export class AnalysisService {
  ...
  async generateMonthlyReport(userId: string, targetDate: Date) {
    ...
    const currentMonthStats = await this.getMonthlyStats(
      userId,
      startOfMonth,
      endOfMonth,
    );
    ...
    // 4. Comparativos
    const comparisonPrev = this.calculateDeltas(
      currentMonthStats,
      prevMonthStats,
    );
  ...
  private generateInsights(current: any, prev: any, avg: any) {
    ...
    if (current.balance > 0)
      texts.push(
        `🟢 Você fechou o mês no azul! Saldo positivo de R$ ${current.balance.toFixed(2)}.`,
      );
    else
      texts.push(
        `🔴 Atenção: Déficit de R$ ${Math.abs(current.balance).toFixed(2)} este mês.`,
      );

    if (prev.expenseDiff < -10)
      texts.push(
        `📉 Parabéns! Você gastou ${Math.abs(prev.expenseDiff).toFixed(1)}% a menos que mês passado.`,
      );
    if (prev.expenseDiff > 10)
      texts.push(
        `📈 Seus gastos aumentaram ${prev.expenseDiff.toFixed(1)}% em relação ao mês anterior.`,
      );
