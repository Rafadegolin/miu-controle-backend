...
import { MissionCompletedEvent } from '../common/events/gamification.events';
...
export class MissionsService {
  ...
  async ensureWeeklyMissions(userId: string) {
    ...
    this.logger.log(
      `Generated ${selected.length} weekly missions for user ${userId}`,
    );
  }
    ...
    for (const userMission of activeMissions) {
      ...
      if (criteria.type === type) {
        ...
        if (completed) {
          ...
          // Credita o XP da missão de forma desacoplada: o GamificationListener
          // ouve 'mission.completed' e chama awardXp (evita dependência circular
          // MissionsService <-> GamificationService).
  ...
  private async seedDefaultMissions() {
    const defaults = [
      ...
      },
      {
        ...
        xpReward: 150,
        frequency: 'WEEKLY',
        criteria: { type: 'GOAL_CONTRIBUTION', target: 1 },
      },
    ];
