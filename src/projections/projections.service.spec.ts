...
describe('ProjectionsService', () => {
  ...
  const mockPredictionService = {
    ...
    predictCategoryExpense: jest
      .fn()
      .mockImplementation((userId, catId, date) => {
        return {
          predictedAmount: 500,
  ...
  it('should calculate realistic scenario correctly', async () => {
    ...
    const result = await service.calculateCashFlow('user1', {
      months: 3,
  ...
  it('should calculate optimistic scenario correctly', async () => {
    ...
    const result = await service.calculateCashFlow('user1', {
      months: 1,
      scenario: ProjectionScenario.OPTIMISTIC,
    });
  ...
  it('should calculate pessimistic scenario correctly', async () => {
    ...
    const result = await service.calculateCashFlow('user1', {
      months: 1,
      scenario: ProjectionScenario.PESSIMISTIC,
    });
