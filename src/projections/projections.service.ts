...
import {
  ...
  ProjectionScenario,
} from './dto/cash-flow-projection-query.dto';
import {
  CashFlowProjectionResponseDto,
  MonthlyProjectionDto,
} from './dto/cash-flow-projection-response.dto';
...
export class ProjectionsService {
  ...
  async calculateCashFlow(
    ...
    const recurringTransactions = await this.recurringService.findAll(userId, {
      isActive: true,
    });
    ...
    for (let i = 0; i < months; i++) {
      ...
      projections.push({
        ...
        scenarios: {
          optimistic:
            accumulatedBalance + (fixedExpenses - variableExpensesOptimistic), // Rough approx accumulation
          pessimistic:
            accumulatedBalance + (fixedExpenses - variableExpensesPessimistic),
        },
      });
    }
