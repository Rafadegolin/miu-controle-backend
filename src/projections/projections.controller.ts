...
export class ProjectionsController {
  ...
  @Get('cash-flow')
  @ApiOperation({ summary: 'Projeção de fluxo de caixa' })
  @ApiOkResponse({ type: CashFlowProjectionResponseDto })
  async getCashFlow(
    @Req() req,
  ...
  async getBalanceForecast(@Req() req, @Query('months') months?: string) {
    // Simplified endpoint for just final balance
    const result = await this.projectionsService.calculateCashFlow(
      req.user.id,
      {
