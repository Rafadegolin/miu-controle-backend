  ...
  async calculateGoalPlan(userId: string, goalId: string) {
    ...
    if (isViable) {
      plan.recommendations.push(
        `Você tem uma margem de R$ ${margin.toFixed(2)}. Ótimo!`,
      );
      ...
      plan.recommendations.push(
        `Faltam R$ ${Math.abs(margin).toFixed(2)} mensais.`,
      );
      ...
      if (totalPotentialCut >= Math.abs(margin)) {
        plan.recommendations.push(
          'É possível atingir a meta cortando gastos não essenciais!',
        );
        cuts.forEach((c) => {
          plan.actionPlan.push({
        ...
        plan.recommendations.push(
          'Cortes não são suficientes. Sugerimos estender o prazo.',
        );
