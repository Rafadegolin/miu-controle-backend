...
export class GeminiService {
   ...
   * @param modelName - Model name (gemini-1.5-flash or gemini-1.5-pro)
   */
  initializeClient(apiKey: string, modelName: string = 'gemini-1.5-flash'): GenerativeModel {
    if (!apiKey) {
  ...
  async createChatCompletion(
    ...
    try {
      ...
      this.logger.debug(
        `Gemini completion: tokens=${usage.totalTokens}`,
      );
  ...
  private handleGeminiError(error: any): void {
    ...
    if (errorMessage.includes('API key')) {
      ...
      throw new Error('Limite de requisições do Gemini excedido.');
    } else {
      throw new Error('Erro na comunicação com o Gemini AI.');
    }
  }
