...
export class PredictiveAnalyticsService {
  ...
  async generateForecast(userId: string) {
    try {
      ...
      // 1. Get Settings & API Key (with fallback enabled)
      ...
      if (historicalData.length < 3) {
        return {
          available: false,
          reason:
            'Dados insuficientes. Precisamos de pelo menos 3 meses de histórico.',
        };
      }
      ...
      // 4. Generate AI Insights with fallback support
      let aiInsights;
      try {
        aiInsights = await this.generateAiAnalysis(
          userId,
          historicalData,
          mathTrends,
          config.apiKey,
        ...
        if (isQuotaError && !config.isFallback) {
          ...
          if (fallbackConfig) {
            ...
            this.logger.error('No fallback configuration available');
            throw error;
          }
        } else {
          // Not a quota error or already tried fallback, rethrow
          throw error;
        }
      }

      // 5. Save Prediction to Database
      await this.savePrediction(userId, mathTrends, aiInsights, config.model);

      return {
        available: true,
        forecast: aiInsights,
        trends: mathTrends,
        usedFallback: config.isFallback || false,
...
Analise os dados e responda APENAS com JSON válido, SEM texto adicional.

DADOS HISTÓRICOS:
${dataSummary}

TENDÊNCIAS (Regressão Linear):
- Previsão Despesa: R$${trends.predictedExpense.toFixed(2)}
- Previsão Receita: R$${trends.predictedIncome.toFixed(2)}
- Tendência: ${trends.expenseTrendSlope > 0 ? 'Despesas subindo' : 'Despesas caindo'}
...
FORMATO JSON OBRIGATÓRIO:
{
  ...
  "predictedIncome": número,
  "savingsGoal": número sugerido,
  "insights": ["insight1", "insight2", "insight3"],
  "recommendation": "Ação principal em 1 frase"
}

IMPORTANTE: Seja CONCISO. Cada insight máximo 15 palavras.`;
    ...
    if (provider === 'GEMINI') {
      ...
      const response = await this.openAiService.createChatCompletion(
        ...
        model,
        { max_tokens: 1000 }, // Increase token limit for detailed analysis
      );
      ...
      throw new Error(`Provider ${provider} not supported for Analytics.`);
    }
    ...
    try {
      ...
      return {
        ...
        predictedExpense: parsed.predictedExpense || trends.predictedExpense,
        predictedIncome: parsed.predictedIncome || trends.predictedIncome,
        savingsGoal: parsed.savingsGoal || 0,
        insights: Array.isArray(parsed.insights)
          ? parsed.insights
          : ['Sem insights detalhados.'],
        recommendation:
          parsed.recommendation || 'Continue monitorando suas finanças.',
      };
    } catch (e) {
      this.logger.error(
        `Failed to parse AI response: ${aiResponseText.substring(0, 500)}...`,
      );
      return {
  ...
  private async savePrediction(
    ...
    await this.prisma.predictionHistory.create({
      data: {
        ...
        predictedBalance:
          (aiData.predictedIncome || trends.predictedIncome) -
          (aiData.predictedExpense || trends.predictedExpense),
        confidence: 0.85,
  ...
  private async calculateHistoryStats(userId: string, days: number) {
    ...
    const variance =
      transactions.reduce(
        (acc, t) => acc + Math.pow(Number(t.amount) - mean, 2),
        0,
      ) / transactions.length;
   ...
   */
  private async analyzeAnomalyWithAi(
    userId: string,
    transaction: any,
    stats: any,
    zScore: number,
    ...
    if (provider === 'GEMINI') {
      try {
        const client = this.geminiService.initializeClient(apiKey, model);
        const res = await this.geminiService.createChatCompletion(client, [
  ...
  async calculateFinancialHealthScore(userId: string) {
    ...
    const expenses = history.map((h) => h.expense);
    const meanExpense =
      expenses.reduce((a, b) => a + b, 0) / expenses.length || 1;
    const variance =
      expenses.reduce((a, b) => a + Math.pow(b - meanExpense, 2), 0) /
      expenses.length;
    const stdDev = Math.sqrt(variance);
  ...
  async calculateTrendsAnalysis(userId: string, period: '3M' | '6M' | '1Y') {
    ...
    const expenseGrowth = this.calculateGrowthRate(
      history.map((h) => h.expense),
    );
