...
export class PredictionEngineService {
   ...
   */
  async isCategoryVariable(
    userId: string,
    categoryId: string,
  ): Promise<boolean> {
    const history = await this.getCategoryMonthlyHistory(
      userId,
      categoryId,
      12,
    );
  ...
  async predictCategoryExpense(
    ...
    const history6M = await this.getCategoryMonthlyHistory(
      userId,
      categoryId,
      6,
    );
    ...
    const prevYearAmount =
      prevYearHistory.length > 0 ? prevYearHistory[0].amount : 0;
    ...
    */
    ...
    const avg6M =
      values.slice(0, 6).reduce((a, b) => a + b, 0) /
      Math.min(values.length, 6);
    ...
    const seasonality = await this.calculateSeasonalityFactor(
      userId,
      categoryId,
      month.getMonth(),
    );
    ...
    const mean6M = avg6M;
    const variance =
      values.slice(0, 6).reduce((a, b) => a + Math.pow(b - mean6M, 2), 0) /
      values.length;
    const stdDev = Math.sqrt(variance);
  ...
  private async getCategoryMonthlyHistory(
    ...
    const endDate = new Date(
      ...
      0,
    ); // Último dia do mês ANTERIOR ao target
    ...
    transactions.forEach((t) => {
      ...
      if (monthlyData.has(key)) {
        monthlyData.set(
          key,
          (monthlyData.get(key) || 0) + Number(t._sum.amount),
        );
      }
    });
