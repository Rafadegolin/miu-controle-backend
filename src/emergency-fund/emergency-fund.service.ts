...
export class EmergencyFundService {
  ...
  async handleMonthlyRecalculation() {
    ...
    for (const fund of funds) {
      try {
        const monthlyEssential = await this.calculateEssentialExpenses(
          fund.userId,
        );
        if (monthlyEssential > 0) {
          ...
          if (
            Math.abs(newTarget - Number(fund.targetAmount)) /
            ...
            this.logger.log(
              `Updated target for user ${fund.userId} to ${newTarget}`,
            );
          }
  ...
  async setup(userId: string) {
    ...
    return this.createFund(userId);
  }

  /**
   * Cria o fundo de emergência do usuário com o alvo padrão (6 meses de
   * despesas essenciais, mínimo 1000). Usado pelo `setup` explícito e como
   * auto-init no primeiro `contribute` (evita a dança 404 -> setup -> retry).
   */
  private async createFund(userId: string) {
  ...
  async contribute(userId: string, amount: number) {
    // Auto-init: se o fundo ainda não existe, cria com o alvo padrão no
    // primeiro aporte (idempotente para o front — sem 404 -> setup -> retry).
    let fund = await this.prisma.emergencyFund.findUnique({
      where: { userId },
    });
    if (!fund) {
      fund = await this.createFund(userId);
    }
  ...
  async updateSettings(
    userId: string,
    data: { targetAmount?: number; linkedGoalId?: string },
  ) {
  ...
  private async checkMilestones(userId: string, fund: any) {
    ...
    for (const m of milestones) {
      if (monthsCovered >= m && monthsCovered < m + 0.5) {
        // Simple range check to avoid spam
