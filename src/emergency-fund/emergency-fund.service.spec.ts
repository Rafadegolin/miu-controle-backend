...
describe('EmergencyFundService', () => {
  ...
  describe('setup', () => {
    it('should calculate target based on expenses (3-6 months)', async () => {
      ...
      mockPrismaService.emergencyFund.create.mockImplementation(
        (args) => args.data,
      );
  ...
  describe('withdraw', () => {
    it('should allow withdrawal if sufficient funds and notify', async () => {
      ...
      mockPrismaService.emergencyFund.update.mockResolvedValue({
        currentAmount: 4000,
      });
      ...
      expect(
        mockPrismaService.emergencyFundWithdrawal.create,
      ).toHaveBeenCalled();
    ...
    it('should block withdrawal if insufficient funds', async () => {
      ...
      await expect(service.withdraw('user1', 1000, 'Test')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
